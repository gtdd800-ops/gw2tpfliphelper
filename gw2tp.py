...
I18N = {
    # ... (tout le bloc I18N d’origine)
    # >>> OPTI : nouvelles chaînes
...
def persist_snapshot(df_bulk: pd.DataFrame):
    if df_bulk.empty: return
    now = int(time.time()); bucket = now // SNAPSHOT_BUCKET_SECONDS
...
def fetch_metrics_for_ids(ids, hours_window=24, trend_hours=1):
    if not ids: return {}, {}, {}, {}
    with db_connect() as conn:
        now = int(time.time()); since_window = now - hours_window * 3600
        q = ",".join("?" for _ in ids)
        cur = conn.execute(
            f"SELECT id, ts, buy, sell, supply, demand FROM snapshots "
            f"WHERE id IN ({q}) AND ts >= ? ORDER BY id, ts",
            (*ids, since_window)
        )
        rows = cur.fetchall()
    ...
    now = int(time.time()); since_trend = now - trend_hours * 3600
    for id_, arr in data.items():
        ...
        dSup[id_] = sup_now - sup_t; dDem[id_] = dem_now - dem_t
...
def fetch_timeseries_for_id(item_id: int, hours_window: int) -> pd.DataFrame:
    with db_connect() as conn:
        ...
        rows = cur.fetchall()
    if not rows: return pd.DataFrame(columns=["ts","buy","sell","supply","demand"])
    df = pd.DataFrame(rows, columns=["ts","buy","sell","supply","demand"]); df["dt"] = pd.to_datetime(df["ts"], unit="s")
    return df
...
with st.sidebar:
    ...
    # >>> OPTI : contrôles d’optimisation d’achat
    st.header(T("Optimisation d'achat"))
    budget_gold = st.number_input(T("Budget (or, 0 = illimité)"), 0.0, 1e9, 0.0, 0.5)
...
# >>> OPTI : calcul de la quantité optimisée et du profit net optimisé
def compute_optimal_qty(row, budget_gold, horizon_sell_h, safety_pct, show_history):
    ...
    if buy_c <= 0 or profit_c <= 0:
        return 0, 0  # pas rentable / pas achetable
...
if df_all.empty:
    ...
    if not df_all.empty:
        ...
        # Affichage (avec colonnes opti)  >>> OPTI : 2 colonnes ajoutées
        ...
        # Valeurs brutes pour un tri correct  >>> OPTI : champs cachés _qopt et _popt_c
        ...
        <style>
          ...
          .gsc-head, .gsc-row {{
            display:grid;
            /* >>> OPTI : +2 colonnes pour Qté opti & Profit opti */
        ...
        <script>
          ...
          let sortKey = 'popt';  // >>> OPTI : par défaut on met en avant le profit optimisé
          let sortDir = 'desc';
          ...
          function render(data) {{
            ...
            data.forEach(r => {{
              ...
              row.appendChild(c(String(r["{T("Qté optimisée)"}] || r["{T("Qté optimisée")}"])));
              row.appendChild(c(r["{T("Profit net optimisé)"}] || r["{T("Profit net optimisé")}"]));
          ...
          function cmp(a, b, key) {{
            if (key === 'name') return (a["{T("Nom")}"] || '').localeCompare(b["{T("Nom")}"] || '', undefined, {{sensitivity:'base'}});
    ...
    if chart_metric_fr == "Profit Net (PO)":
        ...
        ax.set_xticks(range(len(top20))); ax.set_xticklabels(top20["Nom"], rotation=45, ha="right")
        ax.set_ylabel(T("Profit Net (PO)")); ax.set_title(T("Top 20 (GW2TP) — Profit Net"))
        ...
        ax.set_xticks(range(len(top20))); ax.set_xticklabels(top20["Nom"], rotation=45, ha="right")
        ax.set_ylabel(T("Score")); ax.set_title(T("Top 20 (GW2TP) — Score"))
        st.pyplot(fig, clear_figure=True)
